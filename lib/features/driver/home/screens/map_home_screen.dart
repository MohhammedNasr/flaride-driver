...
class _MapHomeScreenState extends State<MapHomeScreen> {
  ...
  void _onMapCreated(GoogleMapController controller) {
    ...
    if (driverProvider.hasWorkLocation) {
      _moveToWorkLocation(driverProvider.workLocationLat!, driverProvider.workLocationLng!);
    }
  }
  ...
  void _updateMapMarkers(List<AvailableOrder> allOrders) {
    ...
    // Filter valid orders
    final visibleOrders = allOrders.where((o) => !_declinedOrderIds.contains(o.id)).toList();
    
    // Filter orders by distance from work location (10km radius)
    final filteredOrders = visibleOrders.where((order) {
      if (order.restaurant?.latitude == null || order.restaurant?.longitude == null) return false;
      
      // If no work location set, show all orders
      if (_lastWorkLocationLat == null || _lastWorkLocationLng == null) return true;
    ...
    final activeOrders = filteredOrders.where((o) => !_hiddenOrderIds.contains(o.id)).toList();
    ...
    for (final order in filteredOrders) {
      ...
      if (order.restaurant?.latitude == null || order.restaurant?.longitude == null) continue;
      
      final isHidden = _hiddenOrderIds.contains(order.id);
      final isActive = !isHidden && order.id == currentActiveOrder?.id;
      final pickupPos = LatLng(order.restaurant!.latitude!, order.restaurant!.longitude!);
  ...
  bool isActiveOrderValid(AvailableOrder order) {
      return (order.restaurant?.latitude != null && order.restaurant?.longitude != null) ||
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
        ...
        // We want to jump to this order.
         ...
         final orders = driverProvider.availableOrders
             .where((o) => !_declinedOrderIds.contains(o.id) && !_hiddenOrderIds.contains(o.id))
             .toList();
  ...
  Widget build(BuildContext context) {
    return Consumer<DriverProvider>(
      builder: (context, driverProvider, child) {
        ...
        final hasActiveOrder = driverProvider.hasActiveOrderFromApi;
        ...
        return Stack(
          children: [
            ...
            if (isOnline && hasActiveOrder && activeOrderDetails != null)
              _buildActiveOrderCard(activeOrderDetails, driverProvider)
            else if (isOnline && orders.isNotEmpty)
              _buildOrderCards(orders, driverProvider),
            if (isOnline && !hasActiveOrder && orders.isEmpty && driverProvider.hasInitialOrdersFetch)
              _buildNoOrdersMessage(),
            if (isOnline && !driverProvider.hasInitialOrdersFetch)
              _buildLoadingOrders(),
          ],
        );
      },
  ...
  Widget _buildOrderCards(List<AvailableOrder> allOrders, DriverProvider driverProvider) {
    // Filter out declined AND hidden orders for the card view
    final visibleOrders = allOrders.where((o) => !_declinedOrderIds.contains(o.id)).toList();
    final activeOrders = visibleOrders.where((o) => !_hiddenOrderIds.contains(o.id)).toList();
  ...
  Widget _buildFloatingOrderCard(AvailableOrder order, List<AvailableOrder> orders) {
          ...
          Padding(
            ...
            child: Row(
              ...
              children: [
                Row(
                  children: [
                    Text('Respond in ', style: TextStyle(fontSize: 13, color: AppColors.midGray)),
                    Text(
                ...
                Text('${_secondsRemaining}s left', style: TextStyle(fontSize: 13, color: AppColors.midGray)),
              ],
            ),
          ),
              ...
              children: [
                Expanded(
                  child: OutlinedButton(
                    ...
                    child: Text(
                      'Decline',
                      style: TextStyle(fontSize: 15, fontWeight: FontWeight.w600, color: AppColors.darkGray),
                    ),
                  ),
                ),
                const SizedBox(width: 12),
  ...
  Widget _buildActiveOrderCard(OrderDetails order, DriverProvider driverProvider) {
